import type { AgentRecord, MessageRecord } from "./types.js";
...
export class NonPreemptiveMailboxScheduler {
  ...
  private async tickAgent(store: ProjectStore, agent: AgentRecord): Promise<void> {
    ...
    const prompt = renderTurnPrompt(config.task, agent, selected);
...
function renderTurnPrompt(task: string, agent: AgentRecord, messages: MessageRecord[]): string {
  return [
    `# Project Task\n\n${task.trim()}`,
