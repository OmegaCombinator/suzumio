...
The root path `/` serves the Preact-based WebUI built from `webui/`. It calls the API routes above and refreshes periodically. For WebUI development, run `npm run webui:dev` and open `http://127.0.0.1:5173`; Vite proxies `/api` and `/health` to the backend on `39400`. The control room includes project selection, status actions, message composition, agent roster, per-agent history, messages, and a per-tool workspace for tool status/WebUI controls. The project overview refreshes from the lightweight project list; large message bodies, agent histories, tool controls, tool status, and archives are loaded on demand. The current project, view, and selected tool page are encoded in the URL hash so refreshing the browser preserves the selected page. Built-in WebUI controls render project-agent fields such as sender, recipient, conversation participants, and signal target as dropdowns from the current project roster.
