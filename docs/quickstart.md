...
| Tool status | A per-tool workspace with aggregate status, submit report path, and one selected WebUI control page at a time. Built-in message/signal controls use dropdowns populated from the current project agents. Local toolpacks such as `toolpacks/scheduler` and `toolpacks/plan` can add pages for scheduled messages and active plans. |
