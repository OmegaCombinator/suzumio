...
| Tool status | Per-tool workspace，包含 aggregate status、submit report path，并且一次只打开一个 WebUI control page。内置 message/signal controls 会用当前 project agents 填充下拉选择。`toolpacks/scheduler` 这类 local toolpacks 可以添加自己的 scheduled messages 页面。 |
