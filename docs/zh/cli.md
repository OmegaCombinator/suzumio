...
`start` 将项目状态设为 `running` 并立即 tick 一次 scheduler；如果 agent 已有 pending signal，可能马上启动 activation。`send` 从虚拟 sender `user` 创建直接消息、创建 pending `message.created` signal，并 tick。优先级为 `P0`、`P1`、`P2`、`P3`：`P0` 会中断并重启 running target，`P1` 尽量在下一次 tool boundary 投递，`P2` 等下一次 activation 且优先于普通 backlog，`P3` 是普通 queued work。
